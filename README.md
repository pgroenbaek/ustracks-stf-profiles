...
You can also make all Dynatrax track shapes use a profile by default by adding `Dynatrax-*` to the `IncludeShapes` parameter within one of the profiles. For example from `IncludedShapes ( "US3r_*" )` to `IncludedShapes ( "US3r_*, Dynatrax-*" )`.
